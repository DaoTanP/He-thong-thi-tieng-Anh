<?php
    //$cauHoi= $_POST('Cau hoi') or die ("khong chon");
    //$dapAnChon =  $_POST('Choose') or die ("khong chon");
    ...
    if(!isset(  $_GET['per_page']))     
        $_GET['per_page'] = 0;
    $startline = $_GET['per_page'] * $numberline;
    $command1 = "SELECT * FROM `cauhoi`  WHERE 1 LIMIT $numberline OFFSET $startline";  
    
    $command2 = "SELECT * FROM `cauhoi` ";
    $linedata = mysqli_query($conn, $command2)->num_rows;
    $numberpage = ceil($linedata / $numberline);

    //lay du lieu theo ngau nhien
    $command3 = "SET @MaCauHoi = FLOOR(RAND( ) * ($linedata - 40)) + 1;
